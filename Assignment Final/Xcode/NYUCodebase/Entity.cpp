...
void Entity::setupBulletEmitter(float w, float h, const char* textureFileName,
                                float rotate, vec::vec2 scale, direction shootingDirection,
                                vec::vec2 shootingVelocity, unsigned int maxBullets) {
    if (be)
        delete be;
    
    be = new BulletEmitter(w, h, vec::vec2(0.0f), maxBullets);
    ...
    switch (shootingDirection) {
        case TOP:
        {
            be->position = vec::vec2(xTrans, yTrans + height);
            be->setVelocity(shootingVelocity);
        } break;
            
        case RIGHT:
        {
            be->position = vec::vec2(xTrans + width, yTrans);
            be->setVelocity(shootingVelocity);
        } break;
...
void Entity::shoot() { if(be) be->shootABullet(); }
