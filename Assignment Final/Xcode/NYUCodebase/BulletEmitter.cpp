...
void BulletEmitter::draw(ShaderProgram* shader) {
    for (size_t i = 0; i < bullets.size(); i++) {
        matrix.identity();
        ...
        glVertexAttribPointer(shader->positionAttribute, 2, GL_FLOAT, GL_FALSE, 0, &vertices[0]);
        glEnableVertexAttribArray(shader->positionAttribute);
        glVertexAttribPointer(shader->texCoordAttribute, 2, GL_FLOAT, false, 0, &texCoord[0]);
...
bool BulletEmitter::viewCheck(Matrix mvp) {
